...
from app.models.user import SimilarUserResponse, UserInDB, UserResponse
...
async def get_my_similar_users(
    ...
    """
    return await user_service.get_similar_users(
...
async def get_my_recommendations(
    ...
    movie_ids = await interaction_service.get_movie_ids_by_interaction(
        user_id=current_user.id, interaction_type=based_on
    )
    ...
    recommendations = await MovieService.get_recommendations_from_movie_ids(
        movie_ids=movie_ids, user_id=current_user.id, limit=limit
    )
...
@router.get("/me/stats")
async def get_my_stats(current_user: UserInDB = Depends(get_current_user_required)):
    """
