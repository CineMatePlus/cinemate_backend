...
async def get_collection_recommendations(
    ...
    recommendations = await MovieService.get_recommendations_from_movie_ids(
        movie_ids=movie_ids, user_id=user_id, limit=limit
    )
...
async def delete_collection(
    ...
    await collection_service.delete_collection(collection_id, str(current_user.id))
    return None


@router.post(
    "/{collection_id}/movies/{movie_id}", status_code=status.HTTP_204_NO_CONTENT
)
